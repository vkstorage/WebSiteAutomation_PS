...
public class FileGenerator {
	...
	public static void generateIndexHTML(String fileLocation) throws IOException {
		...
		bw.write("<BODY>");
		bw.newLine();
		bw.write("<h1>PS.COM Automation</h1>");
		bw.newLine();
		...
		for (int i = list.length - 1; i >= 0; i--) {
			...
			if (file.isDirectory() && !list[i].equals("surefire")) {
	...
	public static void appendTextToBeginingOfFile(String location, String fileName, String text) throws IOException {
		String existingText = "";
		...
		BufferedReader objBufferedReader = new BufferedReader(new FileReader(oldFile.getAbsolutePath()));
		String line;
	...
	public static void generateBarGraphIndexHTML(String fileLocation) throws IOException {
		File indexFile = new File(fileLocation + "/index.html");
		if (!indexFile.exists()) {
			indexFile.createNewFile();
		}
		File[] fileArray = { new File("src/main/resources/index_part1.txt"), new File(fileLocation + "/runSummary.csv"),
				new File("src/main/resources/index_part2.txt") };
		FileWriter output = new FileWriter(indexFile);
		try {
			for (int i = 0; i < fileArray.length; i++) {
				BufferedReader objBufferedReader = new BufferedReader(new FileReader(fileArray[i].getAbsolutePath()));
				String line;
				...
				while ((line = objBufferedReader.readLine()) != null) {
					if (fileArray[i].getName().contains("runSummary.csv")) {
						...
						values[0] = "'" + values[0] + "'";
						if (firstLine) {
							line = "[" + Arrays.toString(values).substring(1, Arrays.toString(values).length() - 1) + ",'']";
							firstLine = false;
						} else {
							line = ",[" + Arrays.toString(values).substring(1, Arrays.toString(values).length() - 1) + ",'']";
						}
